...
try {
  ...
  // Cria o preço mensal
  const priceMensalObj = await stripe.prices.create({
    product: produto.id,
    unit_amount: Math.round(precoMensal * 100),
    currency: 'brl',
  ...
  if (precoAnual && !isNaN(precoAnual)) {
    priceAnualObj = await stripe.prices.create({
      product: produto.id,
      unit_amount: Math.round(precoAnual * 100),
      currency: 'brl',
