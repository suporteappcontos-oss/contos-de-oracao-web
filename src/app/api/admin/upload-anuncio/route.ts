...
export async function POST(request: Request) {
  try {
    ...
    // Faz o upload direto para o BunnyCDN
    const response = await fetch(`https://${REGION_URL}/${STORAGE_ZONE}/anuncios/${fileName}`, {
      method: 'PUT',
      headers: {
    ...
    const fileUrl = `${PULL_ZONE}/anuncios/${fileName}`;
    ...
    console.error('Erro no upload para o BunnyCDN:', error);
    return NextResponse.json(
