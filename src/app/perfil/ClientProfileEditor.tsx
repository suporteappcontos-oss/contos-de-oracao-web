...
export default function ClientProfileEditor({
  ...
  return (
    <>
      ...
      <button
        onClick={abrirModal}
        className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-300 bg-white/5 border border-white/10 hover:border-[#D4AF37]/50 hover:bg-[#D4AF37]/5 text-white/80 hover:text-white active:scale-95 cursor-pointer shadow-lg animate-fade-in"
      >
          ...
          <div className="relative w-full max-w-xl bg-[#111827] border border-white/5 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-white/40 text-xs mt-1">Atualize seus dados de cadastro e escolha do seu santo protetor</p>
              </div>
              <button
            ...
            <div className="flex flex-col items-center justify-center py-2 relative">
              <div className="relative w-24 h-24 rounded-full overflow-hidden border-2 border-[#D4AF37]/30 shadow-xl bg-black/40">
                <img
              ...
              {tempAvatarUrl && (
                <div className="absolute -bottom-1 bg-[#D4AF37]/15 text-[#D4AF37] px-3 py-0.5 rounded-full border border-[#D4AF37]/25 text-[9px] font-bold uppercase tracking-wider shadow">
                  Santo Ativo
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                maxLength={40}
                className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm focus:border-[#D4AF37] focus:outline-none transition-colors"
              />
            </div>
              ...
              <button
                ...
                className={`flex-1 py-3.5 rounded-xl font-bold text-xs flex items-center justify-center gap-2 border transition-all cursor-pointer ${
                  showAvatarSelector
                    ...
                    : 'border-white/5 bg-white/5 hover:bg-white/10 text-white/90'
                }`}
              >
                <Smile size={15} />
                Trocar Foto
            ...
            {/* Seletor de Avatares dos Santos (Expansível) */}
            {showAvatarSelector && (
              <div className="space-y-5 p-4 rounded-2xl bg-black/35 border border-white/5 animate-slideDown">
                <div className="text-white/50 text-[10px] font-bold uppercase tracking-wider mb-2">Selecione o seu Santo Protetor:</div>
                ...
                {loadingAvatars ? (
                  ...
                  <>
                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-4 max-h-[180px] overflow-y-auto pr-1">
                      {avatars.map((a) => {
                    ...
                    <div className="pt-3 border-t border-white/5">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setTempAvatarUrl(null) // Limpa escolha temporária se digitar
                        }}
            ...
            {/* Ações Finais (Cancelar / Salvar) */}
            <div className="flex gap-4 pt-4 border-t border-white/5">
              <button
                onClick={() => setIsOpen(false)}
                disabled={saving}
                className="flex-1 py-3.5 font-bold rounded-xl text-sm border border-white/5 bg-transparent hover:bg-white/5 transition-all text-white/70 disabled:opacity-50 cursor-pointer"
              >
                Cancelar
              </button>
              <button
